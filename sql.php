...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (isset($_POST['snoEdit']) && isset($_POST['music_nameEdit']) && isset($_POST['filepathEdit'])) {
        ...
        $sql = "UPDATE `album` SET `music_name` = '$music_name' , `filepath` = '$filepath', `coverpath` = '$coverpath' 
        WHERE `album`.`sno` = $sno";
        $result = mysqli_query($conn, $sql);
        if($result){
            $update = true;
...
<?php
    if($insert){
        echo "<script>alert('Success! Your music details have been inserted successfully');</script>";
    }
    ?>
    <?php
    if($delete){
        echo "<script>alert('Success! Your music details have been deleted successfully');</script>";
    }
    ?>
    <?php
    if($update){
        echo "<script>alert('Success! Your music details have been updated successfully');</script>";
    }
    ?>
